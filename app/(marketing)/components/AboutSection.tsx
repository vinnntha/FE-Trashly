...
export default function AboutSection() {
  ...
  const [stats, setStats] = useState<{ totalNasabah: number; totalBeratSampahKg: number }>({
    totalNasabah: 7,
    totalBeratSampahKg: 85,
  });

  useEffect(() => {
    apiClient<{ data: any }>("/dashboard/public-stats")
      .then((res) => {
        if (res?.data) {
          setStats({
            totalNasabah: res.data.totalNasabah || 7,
            totalBeratSampahKg: res.data.totalBeratSampahKg || 85,
          });
        }
      })
      .catch(() => {});
            ...
            <ScrollReveal animation="fade-right" duration={700}>
              <div className="relative rounded-3xl overflow-hidden border border-[#0B636B]/15 shadow-md group">
                ...
                <div className="absolute bottom-6 left-6 right-6 text-white">
                  ...
                  <h4 className="font-display font-bold text-lg text-white">
                    Diberdayakan di Unit Sekolah & RW Mitra
                  </h4>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                <div className="p-3.5 rounded-2xl bg-white border border-[#0B636B]/10 shadow-2xs">
                  <p className="text-xs text-[#0B636B]/70 font-medium">Nasabah Terdaftar:</p>
                  <p className="font-display font-extrabold text-xl text-[#0B636B] mt-0.5">
                    {stats.totalNasabah > 0 ? `${stats.totalNasabah} Warga` : "Terdaftar"}
                  </p>
                  <span className="text-[11px] text-[#64B60A] font-semibold">Tergabung di aplikasi</span>
                </div>
                <div className="p-3.5 rounded-2xl bg-white border border-[#0B636B]/10 shadow-2xs">
                  <p className="text-xs text-[#0B636B]/70 font-medium">Sampah Terpilah:</p>
                  <p className="font-display font-extrabold text-xl text-[#64B60A] mt-0.5">
                    {stats.totalBeratSampahKg > 0 ? `${stats.totalBeratSampahKg} kg` : "Tervalidasi"}
                  </p>
